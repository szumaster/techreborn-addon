...
import techreborn.blocks.misc.BlockDrillTube;
import techreborn.config.TechRebornConfig;
import techreborn.init.TRBlockEntities;
import techreborn.init.TRContent;
import techreborn.items.IQuarryUpgrade;
import techreborn.utils.SlotGroup;

import static techreborn.blocks.machine.tier2.QuarryBlock.*;
...
public class QuarryBlockEntity extends PowerAcceptorBlockEntity implements IToolDrop, InventoryProvider, BuiltScreenHandlerProvider {
	...
	public boolean getMineAll() {
		switch (TechRebornConfig.quarryAccessibleExcavationModes){
			case 1: // ores only
			return false;
			case 2: // all only
			return true;
	...
	private BlockPos findOrePos() {
		final int radius = (int)Math.round(TechRebornConfig.quarrySqrWorkRadiusByUpgradeLevel.get(rangeExtenderLevel));
	...
	private void tryDrillDownTube() {
		...
		ItemStack drillTubeItem = new ItemStack(Item.fromBlock(TRContent.DRILL_TUBE));
		...
		setExcavationState(ExcavationState.InProgress);
		world.setBlockState(blockPos, TRContent.DRILL_TUBE.getDefaultState());
	}
	...
	private void tryDrillOutTube() {
		...
		ItemStack tubeItem = new ItemStack(Item.fromBlock(TRContent.DRILL_TUBE));
	...
	private int getTiksPerExcavation() {
		return Math.max((int) (TechRebornConfig.quarryTickPerExcavation * (1d - getSpeedMultiplier())), TechRebornConfig.quarryMinTickPerExcavation);
	}

	private double getEnergyPerExcavation() {
		return TechRebornConfig.quarryEnergyPerExcavation * getPowerMultiplier();
	}
	...
	boolean isOreCheckId(String id) {
		return id.endsWith("_ore") 
			|| TechRebornConfig.quarryAdditionalBlocksToMine.contains(id);
	}
	...
	public double getBaseMaxPower() {
		return TechRebornConfig.quarryMaxEnergy;
	}
	...
	public double getBaseMaxInput() {
		return TechRebornConfig.quarryMaxInput * (1d + getSpeedMultiplier() * TechRebornConfig.quarryMaxInputOverclockerMultipier);
	}

	@Override
	public ItemStack getToolDrop(PlayerEntity entityPlayer) {
		return TRContent.Machine.QUARRY.getStack();
	}
